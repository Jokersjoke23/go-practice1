...
func thirdMiddleware(f func(http.ResponseWriter, *http.Request)) func(http.ResponseWriter, *http.Request) {
	return func(writer http.ResponseWriter, request *http.Request) {
		fmt.Println("3 - middleware (before handler) ---> ")
		f(writer, request) // Handler
		fmt.Println("<--- 3 - middleware (after handler)")
	}
}
...
func main() {
	...
	router.HandleFunc("/korzina", firstMiddleware(secondMiddleware(thirdMiddleware(func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprintf(w, "This is the korzina page")
	}))))
